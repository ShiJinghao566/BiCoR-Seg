...
class LoveDATrainDataset(Dataset):
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        ...
        # MODIFIED: Added a filter to include only files with the correct suffix.
        urban_img_ids = [(str(id.split('.')[0]), 'Urban') for id in urban_img_filename_list if id.endswith(self.img_suffix)]
        ...
        # MODIFIED: Added a filter to include only files with the correct suffix.
        rural_img_ids = [(str(id.split('.')[0]), 'Rural') for id in rural_img_filename_list if id.endswith(self.img_suffix)]
